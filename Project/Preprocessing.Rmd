...
### 1.1 Objectives and Required Informations for the Analyses

Before the analysis we need to upload required packages which are given in the below:
...
#Required packages
pti <- c("data.table", "tidyverse", "lubridate", "tm", "knitr", "tinytex", "scales")
...
Our data set which we obtain from the [kaggle](https://www.kaggle.com/alpertemel/turkey-car-market-2020) is created by using Turkish language. For this reason, before the explanatory analysis, we transform our name of the columns and variables to English. For this, we use following code. 
...
We can add the year and month of advertisements in `Year` and `Month` column.
```{r Date}
...
```{r drop HP NAs and bind,include=FALSE, eval=FALSE}
...
Now, our data is ready for making further analysis. So, we need to export the data for our Rmarkdown.

```{r data export, eval = FALSE}
write.csv(carmarket, file = "turkey_car_market_EDA.csv")
```
